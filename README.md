...
- HTTP 与 HTTPS Keeper 地址；HTTP 仅建议用于可信网络。

## 安全与数据边界

- 管理员密码按规范化 Keeper URL 分别存入 macOS Keychain，不写入源码、配置文件或安装包。
- 会话 Cookie 只保存在应用进程内存中；应用重启后使用 Keychain 密码重新登录。
- 除登录、退出登录以及读取限额缓存所需的 POST 外，监控接口均为只读 GET。
- 应用只读取 Keeper 已有的限额缓存，不触发限额刷新、重置或巡检。
- 客户端包含明确的请求路径与 HTTP Method 白名单，不开放任意管理 API 调用。
- 打包流程会扫描 DMG 内 App，确认没有 `CPA_PWD`、会话 Cookie 或 `.env` 等凭据标记。
- HTTP 传输不加密，管理员密码和监控数据可能被同一网络中的第三方读取；生产环境优先使用 HTTPS。
